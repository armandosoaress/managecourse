@php
    ...
    $langName = \App\Models\Language::where('code',$currantLang)->first();
        ...
        <nav class="navbar navbar-expand-md navbar-light default">
          <div class="container-fluid pe-2">
            <a class="navbar-brand mr-lg-4 pt-0" href="{{route('store.slug',$store->slug)}}">
                @if(!empty($store->logo))
                    <img class="img_setting" alt="Image placeholder" src="{{\App\Models\Utility::get_file('uploads/store_logo/'.$store->logo.'?'. time())}}" id="navbar-logo" style="height: 40px;">
                @else
                    <img class="img_setting" alt="Image placeholder" src="{{asset(Storage::url('uploads/store_logo/logo.png'.'?'. time()))}}" id="navbar-logo" style="height: 40px;">
                @endif
            </a>
            <div class="d-lg-inline-block">
                <span class="navbar-text mr-3 pt-3 text-lg align-middle">{{ucFirst($langName->fullName)}}</span>
            </div>
    ...
    <div id="invoice_logo_img" class="d-none">
        <div class="row align-items-center py-2 px-3">
            @if(!empty($store->invoice_logo))
                <img alt="Image placeholder" src="{{\App\Models\Utility::get_file('uploads/store_logo/'.$store->invoice_logo)}}" id="navbar-logo" style="width: 300px;">
            @else
...
@php
    $store_settings = \App\Models\Store::storeSlug($store->slug);
@endphp
