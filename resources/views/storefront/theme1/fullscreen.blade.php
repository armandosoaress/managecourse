...
@section('content')
    @php
        $userstore  = \App\Models\Store::userStore($store->id);
        $i = 0;
    @endphp
                        ...
                        <div class="pdp-main-media {{!empty($current_chapter->type == 'Video File')? $current_chapter->type == 'Video File':'chapter-video'}}">
                            @if($current_chapter->type == 'Video File')
                                <video id="example" class="preview_video" height="500" poster="{{asset('assets/img/video-img.png')}}">
                                    <source src="{{\App\Models\Utility::get_file('uploads/chapters/'.$current_chapter->video_file)}}" type="video/mp4">
                                    ...
                                    @foreach($practices_files as $practices_file)
                                        <div class="course-download-card" data-id="{{$practices_file->id}}">
                                            <div class="course-image">
                                                <a class="action-item" href="{{\App\Models\Utility::get_file('uploads/practices/'.$practices_file->files)}}">
