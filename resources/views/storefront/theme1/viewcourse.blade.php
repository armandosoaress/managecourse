...
@php
    $profile = \App\Models\Utility::get_file('uploads/default_avatar/avatar.png');
@endphp
@section('meta-data')
    <!-- Primary Meta Tags -->
    ...
    <meta property="og:image" content="{{\App\Models\Utility::get_file('uploads/meta_image/'.$courses->meta_image)}}">
    @endif
    <!-- Twitter -->
    ...
    @if(!empty($courses->meta_image))
        <meta property="twitter:image" content="{{\App\Models\Utility::get_file('uploads/meta_image/'.$courses->meta_image)}}">
    @else
                            ...
                            @if($course->is_preview == 'on')
                                @if($course->preview_type == 'Video File')
                                    <video id="example" class="preview_video" poster="{{asset('assets/imgs/video-img.png')}}" controls controlsList="nodownload">
                                        <source src="{{\App\Models\Utility::get_file('uploads/preview_video/'.$course->preview_content)}}" type="video/mp4">
                                ...
                                <div class="user-review-content">
                                    <div class="abt-user">
                                        <div class="abt-usr-image">
                                            <img @if($course->tutor_id->avatar)src="{{\App\Models\Utility::get_file('/uploads/profile/'.$course->tutor_id->avatar)}}"@else src="{{asset('assets/img/user.png')}}"@endif alt="user">
                                        </div>
                                        <div class="user-info">
                                            <h4>
                                ...
                                <div class="course-media">
                                    <a href="{{route('store.viewcourse',[$store->slug,\Illuminate\Support\Facades\Crypt::encrypt($course_by_cat->id)])}}">
                                        @if(!empty($course_by_cat->thumbnail))
                                            <img src="{{\App\Models\Utility::get_file('uploads/thumbnail/'.$course_by_cat->thumbnail)}}" alt="card">
                                        @else
                                ...
                                @if($homepage_email_subscriber_box == 'on')
                                    ...
                                    @else
                                        @if(!empty($store->sub_img))
                                            <img src="{{\App\Models\Utility::get_file('uploads/store_logo/'.$store->sub_img)}}" alt="newsletter">
                                        @else
