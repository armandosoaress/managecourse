...
trait ZoomMeetingTrait
{
    ...
    public function getToken()
    {
        ...
        if (
            isset($store_settings['zoom_account_id']) && !empty($store_settings['zoom_account_id']) &&
            isset($store_settings['zoom_client_id']) && !empty($store_settings['zoom_client_id']) &&
            isset($store_settings['zoom_client_secret']) && !empty($store_settings['zoom_client_secret'])
        ) {
            // Construct the basic authentication header
            $basicAuthHeader = base64_encode($store_settings['zoom_client_id'] . ':' . $store_settings['zoom_client_secret']);
            ...
            $response = $this->client->request('POST', 'https://zoom.us/oauth/token', [
                ...
                'form_params' => [
                    ...
                    "account_id" =>  $store_settings['zoom_account_id']
                ],
            ]);
