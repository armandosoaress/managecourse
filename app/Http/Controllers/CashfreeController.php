            ...
            try {
                if (!empty($request->coupon)) {
                    ...
                    $data = json_encode([
                        ...
                        "order_currency" => !empty($admin_payment_setting['currency']) ? $admin_payment_setting['currency'] : 'USD',
        ...
        try {
            ...
            if ($products)
            {
                ...
                $data = json_encode([
                    ...
                    "order_name" => $student['name'],
                    "customer_details" => [
                        "customer_id" => 'customer_' . $student['id'],
                        "customer_name" => $student['name'],
                        "customer_email" => $student['email'],
                        "customer_phone" => '1234567890',
                    ],
                    "order_meta" => [
                        "return_url" => route('store.cashfreePayment.success') . '?order_id={order_id}&order_token={order_token}&product_id=' . implode(',',$product_id) . '&amount=' . $total_price . '&coupon=' . $coupon  . '&slug=' . $slug  . ''
                    ]
                ]);
                try {
